...
class APIClient:
    ...
    def post_status(self, status):
        if not self.robot_id:
            self.logger.warning("No robot_id set, skipping post_status.")
            return
        url = f"{self.api_url}/{self.robot_id}/status"
        try:
            resp = requests.post(url, json=status)
            self.logger.info(f"Status post: {resp.status_code}")
        except Exception as e:
            self.logger.error(f"Error posting status: {e}")

    def get_commands(self):
        if not self.robot_id:
            self.logger.warning("No robot_id set, skipping get_commands.")
            return []
